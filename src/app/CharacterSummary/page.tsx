...
const STORAGE_KEY = "StatsTalents"
...
export default function CharacterSummary() {
  ...
  const get = (key: string, multiplier = 1) => ((stats[key] ?? 0) * multiplier).toFixed(0)
          ...
          <tbody>
            <tr className="text-center">
              {["ATK", "DEF", "MATK", "HEAL"].map(stat => (
                <td key={stat} className={statStyle(stats[stat] ?? 0)}>{get(stat)}</td>
              ))}
            </tr>
            <tr className="font-bold text-center">
              {["ATK%", "DEF%", "MATK%", "HEAL%"].map((stat, i) => (
            ...
            <tr className="text-center">
              {["ATK%", "DEF%", "MATK%", "HEAL%"].map(stat => (
                <td key={stat} className={statStyle(stats[stat] ?? 0)}>{get(stat, 100)}%</td>
              ))}
            </tr>
            <tr className="text-center">
          ...
          <tbody>
            {["Blunt", "Pierce", "Slash", "Fire", "Water", "Lightning", "Wind", "Earth", "Toxic", "Neg", "Holy", "Void"].map(type => (
              <tr key={type} className="text-center">
                ...
                <td className={statStyle(stats[`${type}%`] ?? 0)}>{get(`${type}%`, 100)}%</td>
                <td className={statStyle(stats[`${type} Pen%`] ?? 0)}>{get(`${type} Pen%`, 100)}%</td>
                <td className={statStyle(stats[`${type} Res%`] ?? 0)}>{get(`${type} Res%`, 100)}%</td>
                <td className={statStyle(stats[`x${type}%`] ?? 0)}>{get(`x${type}%`, 100)}%</td>
                <td className={statStyle(stats[`${type} xPen%`] ?? 0)}>{get(`${type} xPen%`, 100)}%</td>
                <td className={statStyle(stats[`${type} Skill%`] ?? 0)}>{get(`${type} Skill%`, 100)}%</td>
              </tr>
            ))}
          </tbody>
        </table>
